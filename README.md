...
**能力感知（不再回答「做不到了」）**：每次对话前，系统会把当前**已启用**的插件清单（名称 / 类型 / 运行状态 / 描述）注入对话上下文；当 BOT Self Coding 开启时，还会告知智能体具备「自主构建新插件」的能力。因此面对需求时，智能体会**优先调用已有插件**，或在暂无现成能力时**主动声明要为其构建插件并提交开发任务**（而非回答「做不到了 / 我做不到 / 不支持」）。该提示在无已启用插件且未开启自我编程时自动省略，零额外开销。
...
4. **装载并上架插件页**：构建通过且 `AUTO_LOAD=True` → 自动 `pkg_manager.load()` 并启动（brain/world 类型按 `auto_start_on_core`），同时向「插件」页**推送刷新事件**、并在聊天里提示「✅ 已自动装载并启动插件「`<name>`」」——无需手动刷新即可在插件页看到并管理；`AUTO_LOAD=False` → 只回报「建议装载 `<name>`」，由用户允许或手动 `/装载 <name>`。
...
- **想让肥鱼自己做一个功能 / 插件**：在配置页「智能体自编程」或发 `/开启自我编程` 打开 BOT Self Coding，然后直接对它说「帮我做个能 XXX 的插件」即可。智能体识别到暂无现成能力时会自动提交开发任务、交给构建助手构建，完成后自动装载并出现在「插件」页（聊天里也会提示）；过程中可在「自编程 Issue」页查看实时构建日志、或「重提 / 同意 / 拒绝」Issue。
