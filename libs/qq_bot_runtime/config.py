...
# ---- QQ 群监听白名单（默认关闭）----
# 仅当本列表非空时生效：只有列表内的群号会被处理（组装消息 / 分发核心 / 回复），
# 其余群的群消息在分发入口直接忽略（不组装、不调用 LLM、不回复）。
# 与 B 站 DM 白名单（BILIBILI_DM_WHITELIST）对齐的安全模型：白名单之外的入口不进入大脑。
# 私聊不受影响（仍走 ONLY_MENTION_OR_PRIVATE 门控）。
# 填群号字符串，例如 ["123456789"]；空列表 = 关闭白名单（所有群都处理，保持旧行为）。
QQ_GROUP_WHITELIST = []
