...
## 8. 动图（GIF/动画 WebP）识别失败（2026-09-21 已修复）

- **现象**：发 QQ 动图后机器人「识别失败」；运行日志 `[ERROR] 图片识别失败: 无法获取图片`。
- **根因（两处）**：
  1. `qq_plugin.get_image_bytes`：QQ 动图常无直链 `url`，需 `get_image` 缓存到本地；原实现只尝试读一次本地文件，动图/大图缓存未就绪即放弃 → `RuntimeError("无法获取图片")`。
  2. `chat_service._chat_pipeline` 图片分支（原 1511-1538）：把「视觉识别」和「表情包收集」裹在同一个 `try` 里，视觉异常被外层 `except` 以 `[WARN] 表情包收集失败` 吞掉，`user_content` 始终拿不到图片描述 → 表现为「动图识别失败」；且 GIF 视觉异常无退回路径。
- **修复**：
  - `get_image_bytes` 加 4 次重试 + 每次 1.2s 短等待，本地文件为空/暂不存在时重试；`url` 增加 `http(s)` 校验避免脏值；`get_image` 返回的 `new_url` 同样校验后下载。
  - 图片分支重构为三段独立 try：**取图 → 视觉识别 → 表情包收集**。视觉识别里 GIF / **动画 WebP** 走 `describe_gif_animation`（多帧），动图多帧失败即退回单帧 `describe_image`；任一视觉失败打 `[ERROR] 图片识别失败` 并 `continue`，不再被表情包收集警告掩盖。WebP 也走多帧路径（PIL 抽帧转 JPEG，比直接发 webp 更稳）。
- **教训**：多步骤管线里「取资源 / 识别 / 后处理」应分段 try，避免前段异常被后段 except 误吞；平台媒体缓存未就绪要重试而非一次性放弃。
